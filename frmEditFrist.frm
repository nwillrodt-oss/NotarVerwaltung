...
Begin VB.Form frmEditFrist 
   ...
   Begin VB.CommandButton cmdUpdate 
      ...
      Width           =   1215
   End
   Begin VB.TextBox txtFristdatum 
      DataField       =   "FRIST024"
   ...
   Begin VB.CommandButton cmdSave 
      ...
      Width           =   375
   End
   Begin MSComCtl2.DTPicker dtFristdatum 
      BeginProperty DataFormat 
         Type            =   1
         Format          =   "dd.MM.yyyy"
         HaveTrueFalseNull=   0
   ...
   End
End
Attribute VB_Name = "frmEditFrist"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit                                                     ' Variaben Deklaration erzwingen
Private Const MODULNAME = "frmFrist"                                ' Modulname für Fehlerbehandlung
...
Private szRootkey As String                                         ' = Frist
...
Private rsFrist As ADODB.Recordset
...
On Error Resume Next                                                ' Fehlerbehandlung deaktiviern
    rsFrist.Close                                                   ' RS Bewerbung schliessen
...
'        StellenID = tmpArray(1)
    ...
    If bNew Then                                                    ' Wenn DS Neu
        ...
        txtID = ThisDBCon.GetValueFromSQL("SELECT NewID()")         ' Neue ID (Guid) ermitteln
        szID = txtID
        txtIDBewerber.Text = PersID
        Call FormatDTPicker(Me, dtFristdatum, Now())
        Call dtFristdatum_Change
    ...
    Call InitFrameFristDaten(True)                                  ' Frame Fristdaten Initialisieren
    Call InitFrameInfo(Me)                                          ' Info Frame initialisieren
    Call RefreshRelFields                                           ' zu den FK IDs relevate Werte holen
    Call SetEditFormCaption(Me, szRootkey, txtFristdatum)           ' Formular Caption abhängig von den Daten setzen
...
Private Sub InitFrameFristDaten(Optional bVisible As Boolean)
On Error Resume Next                                                ' Fehlerbehandlung deaktivieren
    Call PosFrameAndListView(Me, FrameFrist, True)                  ' Frame Positionieren
    FrameFrist.Visible = bVisible                                   ' Sichtbar ?
    Err.Clear                                                       ' Evtl. Error clearen
End Sub
...
Errorhandler:
    ...
    Err.Clear                                                       ' Fehler Clearen
On Error Resume Next                                                ' Keinen Fehler in de rFehlerbehandlung zulassen
    Call objError.Errorhandler(MODULNAME, "RefreshRelFields", errNr, errDesc) ' Fehler behandlung aufrufen
    Resume exithandler                                              ' Weiter mit Exithandler
End Function
...
On Error GoTo Errorhandler                                          ' Fehlerbehandlung aktivieren
    ...
    If TS.SelectedItem = "Info" Then                                ' Tab DS info
        FrameFrist.Visible = False                                  ' Frame Frist ausblenden
        FrameInfo.Visible = True                                    ' Frame Info einblenden
        Exit Sub
    End If
    Select Case TS.SelectedItem.Index
    Case 1                                                          ' 1. Tab. Frist daten
        FrameFrist.Visible = True                                   ' Frame Frist einblenden
        FrameInfo.Visible = False                                   ' Frame Indo ausblenden
    Case 2                                                          ' 2. Tab. DS info
        FrameFrist.Visible = False                                  ' Frame Frist ausblenden
        FrameInfo.Visible = True                                    ' Frame Info einblenden
    Case Else
...
On Error GoTo Errorhandler                                          ' Fehlerbehandlung aktivieren
    ...
    bValidationFaild = ValidateTxtFieldOnEmpty(txtFristdatum, "Datum d. Bewerbung", _
            szMSG, FocusCTL)                                        ' txtFristdatum auf Leer prüfen
' MW 14.02.11 Bemerkungs feld validieren {
    bValidationFaild = ValidateTxtFieldOnEmpty(txtBem, "Bemerkung", _
            szMSG, FocusCTL)                                        ' txtBem auf Leer prüfen
' MW 14.02.11 }
...
End Sub
                                                                    ...
                                                                    ' *****************************************
                                                                    ' Change Events
Private Sub txtFristdatum_Change()
On Error Resume Next                                                ' Fehlerbehandlung deaktivieren
    If Len(txtFristdatum.Text) < 8 Then Exit Sub                    ' Text zur kurz -> nicht weitermachen
    If IsDate(txtFristdatum.Text) Then                              ' Ist Text Datum?
        dtFristdatum.Value = txtFristdatum.Text                     ' Datum an DT übergeben
...
Private Sub dtFristdatum_Change()
On Error Resume Next                                                ' Fehlerbehandlung deaktivieren
    txtFristdatum.Text = dtFristdatum.Value                         ' Datum an txt Feld übergeben
    If bInit Then Exit Sub                                          ' Wenn Form initialisiert -> fertig
    Me.Adodc1.Recordset.Fields(txtFristdatum.DataField).Value _
            = Format(Me.dtFristdatum.Value, "dd.mm.yyyy")           ' Bei DTPicker Wert nochmal übernehmen da Sonst die Datenbindung nicht funzt
    Err.Clear                                                       ' Evtl. error clearen
End Sub
...
Private Sub txtFristdatum_GotFocus()
    Call HiglightCurentField(Me, txtFristdatum, False)              ' Hervorhebung Aktiv anschalten
End Sub

Private Sub txtFristdatum_LostFocus()
    Call HiglightCurentField(Me, txtFristdatum, True)               ' Hervorhebung Aktiv abschalten
End Sub
                                                                    ' *****************************************
                                                                    ' Key Down Events
